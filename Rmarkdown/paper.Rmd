...
$$(2)~~f(t) = \sum_{j=1}^kb_j(t)\beta_j$$
$b_j(t)$ are the basis functions of $t$ that comprise $f(t)$, $\beta_j$ are the model coefficients (i.e., the basis function weights), and $\epsilon_t$ is the error term. We fitted the example GAM in Figures 2--3 using the R package *mgcv* [@mgcv], which uses penalized likelihood maximization instead of ordinary least squares to choose coefficients while protecting against overfitting.
...
Left-censored observations are indicated by the variable $censored = 1$ and were modeled using $F(x)$, the cumulative Student *t*-distribution (*i.e.*, $P(X \leq l)$, the probability that, given a set of distributional parameters, a random variable $X$ is equal to the censoring limit, $l$, *at most*) [@stan2021; @mcelreath_statistical_2016]. Interval-censored observations are indicated by $censored = 2$ and were modeled using $g(l, u)$, or $P(l \lt X \leq u) = F_X(u) - F_X(l)$, where $l$ and $u$ are the lower and upper limits, respectively. Parameters, then, were estimated without imputation of nondetects: at each iteration of the MCMC algorithm, censored observations factor into the computed posterior via the cumulative probability distribution, $F(l)$, only.
...
\sigma_{\alpha} \sim Half \text{-} T(0, 2.5, 3)~~[prior~on~\sigma~of~pipes] \\ 
\end{align}
$$
...
\sigma_b \sim  \text{-} T(0, 2.5, 3) (global/group \text{-}level~smoothers) \\ 
\end{align}
$$
...
The decrease and subsequent increase in particulate lead beginning in late 2020 are not easily explained, but they may be related to water temperature. Seasonal temperature variation was most pronounced in the final 1.5 years of the study (Figure 8c), coinciding approximately with the apparent seasonal pattern in particulate lead (Figure 8a). Increasing feedwater aluminum concentrations may also have been a factor (Figure S9)---previous work has shown that aluminum and orthophosphate interact to increase particulate lead release [@trueman_seasonal_2022]. 
