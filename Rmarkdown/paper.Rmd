...
```{r figure-4, fig.cap="**Figure 4. (a)** Seasonal component of the trend in dissolved lead. **(b)** Global longterm component of the trend in dissolved and particulate lead. **(c)** Group-level (i.e., pipe-specific) longterm trends in dissolved and particulate lead, representing the deviations of each time series from the global longterm trend. In panels **(a--b)**, shaded regions span the middle 95% of the posterior distribution, and 200 draws from that posterior are superimposed in blue. All plots are shown on the transformed scale and centered."}
knitr::include_graphics(here::here("Rmarkdown/figures/figure-4.png"))
```

The models included one-dimensional global smooths describing longterm variation in lead across all time series (Figure 4b). The model of dissolved lead also included a global smooth describing seasonal variation (Figure 4a). Both models included group-level smooths describing longterm variation in lead release from each pipe (Figure 4c), a hierarchical structure that is discussed in a recent paper [@pedersen_hierarchical_2019]. The longterm smooths were thin-plate regression splines and the seasonal smooth was a cyclic cubic regression spline [@mgcv]. To speed up computation, the response variable was log-transformed and then scaled to have zero mean and unit variance.
...
Time series of particulate and dissolved lead featured prominent negative trends in the first year of operation followed by an extended period of relative stability. The dissolved lead time series were seasonal, with maximum levels in late summer and minimum levels in early spring (Figure 4a). To estimate the time-varying mean representing each series, we modeled lead as the sum of a global longterm smooth, a global seasonal smooth (dissolved lead only), a group-level longterm smooth, a local intercept for each pipe, and a continuous-time autocorrelated error term (Figure 6).
