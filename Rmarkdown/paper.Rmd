...
Here, we propose a Bayesian approach for building statistical models of pipe loop study data that is sufficiently flexible for most scenarios. The collection of measurement series representing variation in a given parameter over time---often lead--- is represented as the sum of five components: 

1. An average representing each pipe
2. A global trend describing long-term variation in the parameter of interest across all pipes
3. A set of pipe-specific trends that describe the deviations from the global long-term trend particular to each individual pipe
...
5. An error autocorrelated term

Separation of the time-trend into long-term and seasonal components allows for easier identification of a stable phase in the presence of significant seasonality, and separation of the long-term trend into global and pipe-specific components facilitates trend estimation in the presence of censoring or where there are longer data gaps the series representing one or more pipes. In these instances, the model "borrows" information from the global term. The autocorrelated error model improves the accuracy of parameter distributions and helps to prevent overfitting.
...
The models included one-dimensional global smooths describing long-term variation in lead across all time series (Figure 4b). The model of dissolved lead also included a global smooth describing seasonal variation (Figure 4a). Both models included group-level smooths describing long-term variation in lead release from each pipe (Figure 4c), a hierarchical structure that is discussed elsewhere [@pedersen_hierarchical_2019]. The long-term smooths were thin-plate regression splines and the seasonal smooth was a cyclic cubic regression spline [@mgcv]. To speed up computation, the response variable was log-transformed and then scaled to have zero mean and unit variance.

Time series representing lead release to drinking water tend to feature extreme values caused by particulate or colloidal lead. These can severely bias parameter estimates unless they are accounted for. And since our dataset was impacted by extreme lead release events, we used a Student *t* likelihood in place of the usual Gaussian likelihood to limit the influence of outliers [@brms; @brms_adv].
...
The transformed data, then, ($y_t$) were modeled as follows:
