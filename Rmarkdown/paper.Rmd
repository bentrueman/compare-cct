...
Pipe loop studies are used to model drinking water distribution systems, mainly for evaluating corrosion control treatment. The US EPA's Revised Lead and Copper Rule is expected to result in many water utilities conducting pipe loop studies [@estessmargiassi_understanding_2020], and the data they generate will need to be analyzed. But pipe loop study data are inherently complex and there is a real risk of misinterpreting them. The following are just some of the challenges that arise (Figure 1): 
...
2. A set of group-level trends that describe the deviations from the global trend particular to the individual pipes.
3. A global seasonal trend, whose strength depends on the magnitude of the seasonal variation in water quality and temperature. (This component may not be necessary if there is no distinct seasonality.)
4. An autocorrelated error term.

We demonstrate this approach using an example pipe loop dataset representing four years of data, multiple pipe configurations, and multiple orthophosphate dosing protocols. Our model estimates autocorrelation, despite an irregular sampling frequency; it also accommodates censored observations without imputation and is robust against extreme analyte concentrations due to particle release---meaning that no special treatment of outliers is necessary. An advantage of our Bayesian approach is the ability to move beyond point estimates and intervals: Bayesian models yield the full posterior distribution of all parameters, and all quantities derived from those parameters, including the predicted values. This means that we can construct any comparison we wish to investigate, without the need to decide on a particular statistical test. Moreover, the use of additive smoothing splines permits a description of time series using multiple nonlinear components, which can yield a more nuanced understanding of differences in lead concentrations over time.  
...
The generalized additive model (GAM) does better than the constant or even the linear model in accounting for autocorrelation in the data (Figure 2). This is because it accounts for non-linear patterns that the simpler models ignore. GAMs can also be thought of as linear models, but instead of regressing y on x, we regress y on *functions* of x (cubic functions, for example, as in Figure 3). 
...
$$(4)~~h(s, \phi) = \phi^s, s\geq0, \phi\geq0, \phi\leq1 $$
...
\mu_t = \alpha_{pipe_i} + \sum_{j=1}^{n}f_j(t) + \phi^s(y_{t-s} - \mu_{t-s}) \\
\sigma \sim T(0, 2.5, 3) \\
\nu \sim Gamma(2,0.5) \\
\phi \sim N(0.5, 0.25) \\
\end{align}
...
where $T$ represents the Student *t*-distribution with mean $\mu_t$, standard deviation $\sigma$, and degrees-of-freedom parameter $\nu$, determining the difference between the *t*-distribution and a Gaussian with the same $\mu$ and $\sigma$. The autocorrelation coeffient $\phi^s$ is defined in equation $(4)$. 
...
And the $f_j(t)$ are smooth spline functions that comprise the GAM, each with the following form:
...
Time series of particulate and dissolved lead featured prominent negative trends in the first year of operation followed by an extended period of relative stability. The dissolved lead time series were seasonal, with maximum levels in late summer and minimum levels in early spring (Figure 4a). To estimate the time-varying mean representing each series, we modeled lead as the sum of a global multiyear smooth, a global seasonal smooth (dissolved lead only), a group-level multiyear smooth, a local intercept for each pipe, and a continuous-time autocorrelated error term (Figure 6).

Autocorrelation models the dependence of observations on previous observations, or the response of the series at time $t+s$ to a shock at time $t$. The effect of the autocorrelation term here is most evident in the response of the model to high particulate lead release after short sections of lead pipe were removed from the lead-copper pipe copuples in Nov. 2020 ("Pb-Cu", Figure 6). While the time-varying mean $\mu_t$ exhibits pronounced spikes corresponding to particle release events, the underlying GAM remains smooth (Figure 4). That is, extreme and sustained deviations from typical behavior are generally captured by the error term.  
...
The ratio comparing the 1 and 2--0.75 mg P L^-1^ protocols exhibited a more nuanced pattern (Figure 7b). As a comparison of either particulate or lead concentrations, it was less than one over much of the first part of the series and greater than one over much of the latter part. The transition occurred a short time after orthophosphate was decreased from 2--0.75 mg P L^-1^, suggesting that 2 mg P L^-1^ caused excess lead release relative to the 1 mg P L^-1^ dose, likely as particles and small colloids. Meanwhile, this dose presumably caused a scale layer to form that ultimately released less lead---in both fractions---than its counterpart formed at 1 mg P L^-1^, despite being exposed to a lower concentration of 0.75 mg P L^-1^.
...
Effluent lead concentrations in pipe loop studies typically exhibit a pronounced negative trend after commissioning, followed by relative stability for the duration of the study [@wysock_statistical_1995], assuming other inputs to the system remain constant. This general pattern is represented in our model by the global multiyear trend (Figure 4b), which excludes variation in effluent lead due to non-constant inputs (e.g., water quality and temperature). The stability of a pipe array---accounting for effluent from all pipes in the study---can be evaluated by considering the rate of change in this global term. Here, dissolved and particulate lead concentrations in pipe loop effluent declined significantly in the first year of operation as the system stabilized following commissioning (Figure 8a). Both fractions exhibited more modest changes in subsequent years that are difficult to attribute, but they may be a response to changes in water quality over the course of the study.

```{r figure-8, fig.cap="**Figure 8. (a) ** The global multiyear smooth and its estimated first derivative. A random sample of 200 smooths/derivatives is superimposed, and both sets of lines are underlain by shaded regions covering the middle 95% of the respective posterior distributions. The median of the global trend is shaded red when the 95% credible interval around the first derivative does not include zero. **(b--c)** Iron and water temperature, both measured in pipe rack influent, exhibited comparable seasonal patterns to lead, the latter described by the seasonal term in the GAM. Ticks at the top and bottom edges of the plot in **(b)** represent anomalously high and left-censored iron concentrations, respectively."}
...
Supplementary information for this article includes additional description of the pipe loop study, a sensitivity analysis of the priors, models summaries, and convergence statistics.
